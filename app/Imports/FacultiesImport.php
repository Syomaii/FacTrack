...
class FacultiesImport implements 
    ToModel, 
    WithHeadingRow, 
    SkipsOnError, 
    SkipsOnFailure,
    ...
    */
    ...
    {   
        ...
        $user = new User([
            ...
            'email' => $faculty->email,
            'password' => bcrypt($faculty->id), 
            'type' => 'faculty', 
            'status' => 'active',
            'mobile_no' => 'none', 
        ]);

        $faculty->save(); // Save the faculty record first
    ...
    {
        return [
            '*.ID No\.' => ['unique:faculty,id'], // Target the exact header format
        ];
    }
